...
<div class="container1">
    <h3 style="text-align: center; ">Reports-Sales per Stock Item</h3>        
     <hr/>
    ...
    <ul class="navsmall">  
        ...
        <li><a><h4>Sales Report / Stock Item</h4></a></li>  
        <li><a href="reports-sales-custom.php"><h4>Custom Report</h4></a></li>
    </ul><br /><hr />
    <div id="container2">
          ...
          for ($i = ($mon-1); $i >= 1; $i--){
              ...
              echo "'".$date->format('F, Y')."'".",";
               if($st == ""){
                  ...
                  $salesdata[] = Stockhistory::get_total_amount_item(0,$date->format('m'),$date->format('Y'));
                } else {
                  //get upcomin for just one user
                 $salesdata[] = Stockhistory::get_total_amount_item($st,$date->format('m'),$date->format('Y'));
                }              
            }
            if($st == ""){
                  // get upcoming for all users
                  $salesdata[] = Stockhistory::get_total_amount_item(0,date('m'),date('Y'));
                } else {
                  //get upcomin for just one user
                 $salesdata[] = Stockhistory::get_total_amount_item($st,date('m'),date('Y'));
                }
            echo "'".date('F, Y')."'" ?>],
        datasets: [{
            label: "Sales Data Per Stock (GHS)",
            data: [
            <?php  
          for ($i = 0; $i < ($mon-1); $i++){
            echo $salesdata[($i)]. ",";
